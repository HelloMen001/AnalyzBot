...
class AI_RangeModule:
    def __init__(self,window_size,model_file='AI/AI_RangeModel.keras'):
    ...
    def prepare_data(self):
        ...
        for i in range(len(encoded_log) - self.window_size):
            ...
            zone = self.range_map.get(self.raw_log[i + self.window_size][2], 1)
            y.append(to_categorical(zone, num_classes=3))
    ...
    def predict(self):
        ...
        return {
           "параметр": "диапазон",
            "значение": ["LOW", "MID", "HIGH"][predicted_class],
