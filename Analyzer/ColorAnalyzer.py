...
import json
from LogWork import ColorLog_N

class ColorAnalyzer:
    def __init__(self, LenColorLog, min_pattern_len, max_pattern_len, min_count, min_probability):
        self.Log = ColorLog_N(LenColorLog)
    ...
    def analyze(self):
        ...
        for n in range(self.min_len,self.max_len+1):
            for i in range(len(self.Log)-n):
                ...
                next_color = self.Log[i+n]
                pattern_data[pattern].append(next_color)
